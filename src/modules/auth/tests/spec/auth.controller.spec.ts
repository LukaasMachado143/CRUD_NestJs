...
describe('Auth Controller', () => {
  ...
  describe('Authenticated routes', () => {
    ...
    test('method uploadFiles', async () => {
      ...
      const result = await authController.uploadFiles([photo, photo1, photo2, photo3])
    ...
    test('method uploadFilesFields', async () => {
      ...
      const result = await authController.uploadFilesFields(userEntityList[0],filesFields)
      expect(result).toStrictEqual({ message: 'Inserted files' })
    })
  })
})