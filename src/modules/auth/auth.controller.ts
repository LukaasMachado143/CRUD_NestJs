...
export class AuthController {
  ...
  async uploadPhoto(
    ...
    return {
      message: result ? 'Profile photo updated' : 'Profile photo NOT updated',
    };
  }
  ...
  async uploadFilesFields(
    @User() user,
    @UploadedFiles()
    filesFields: { photo: Express.Multer.File; files: Express.Multer.File[] },
  ) {
    const result = await this.fileService.uploadFilesFields(
      filesFields,
      user.id,
    );
    return { message: result ? 'Inserted files' : 'NOT inserted files' };
